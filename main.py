...
def process_dataset(filepath: str):
    ...
    # Clean dirty string values per column dynamically
    clean_cols_sql = ", ".join([
        f"CASE WHEN TRY_CAST(\"{col}\" AS VARCHAR) IN ('ERROR', 'UNKNOWN', 'null', 'None', 'N/A', '') THEN NULL ELSE \"{col}\" END AS \"{col}\""
        for col in cols
    ])
    
    conn.execute(f"CREATE VIEW clean_data AS SELECT {clean_cols_sql} FROM raw_data")
...
async def ask_question(question: str = Form(...)):
    ...
    try:
        ...
        conn.execute(f"CREATE VIEW raw_data AS SELECT * FROM read_csv_auto('{ACTIVE_FILE_PATH}', ignore_errors=true)")
